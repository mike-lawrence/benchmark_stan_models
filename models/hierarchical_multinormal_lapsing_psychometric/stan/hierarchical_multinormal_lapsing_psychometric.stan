...
model{
	...
	Y ~ bernoulli(
	 	p_quick(
			...
			, p_chance[yXc] // data variable
			, one_minus_p_chance[yXc] // data variable
	...
	// vector[rXc] threshold_for_subj_by_cond = sqrt(exp(rows_dot_product( iZc_mat[iXc,1:nXc] , Xc ))) ;
	// vector[rXc] slope_for_subj_by_cond = sqrt(exp(rows_dot_product( iZc_mat[iXc,(nXc+1):nXc2] , Xc ))) ;
	// vector[rXc] p_lapse_for_subj_by_cond = inv_logit(iZc_mat[,nXc21])[iXc] ;
	// vector[nY] p_success = p_quick(
	// 	nY
	// 	, intensity
	// 	, p_chance[yXc]
	// 	, one_minus_p_chance[yXc]
	// 	, p_lapse_for_subj_by_cond[yXc]
	// 	, threshold_for_subj_by_cond[yXc]
	// 	, slope_for_subj_by_cond[yXc]
	// ) ;
	// Y ~ bernoulli( p_success ) ;
	//// BUT you WOULD want to define-then-use if using reduce_sum to speed up the final bernoulli
