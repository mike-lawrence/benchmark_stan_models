...
data{
	...
	// nY: num entries in the observation cells
	int<lower=nI> nY ;

	// zeroes_count: count of zeroes in each cell
	array[nY] int<lower=0> zeroes_count ;

	// ones_count: count of ones in each cell
	array[nY] int<lower=0> ones_count ;
...
transformed data{
	...
	// total_count: sum of ones & zeroes
	array[nY] int total_count ;
	for(i_nY in 1:nY){
		total_count[i_nY] = zeroes_count[i_nY] + ones_count[i_nY] ;
	}
...
model{
	...
	// ones count as binomial logit
	ones_count ~ binomial_logit(
		total_count
		, iZc_dot_Xc[yXc]
	) ;
