...
(
	...
	%>% group_by(c,.add=T)
	%>% summarise(
		value = rbinom(nY_per_ic,1,plogis(value))
		, .groups = 'keep'
	)
) ->
	Y
...
(
	...
	%>% summarise(
		zeroes_count = sum(value==0)
		, ones_count = sum(value==1)
		, .groups = 'drop'
	)
) ->
	dat
...
data_for_stan = lst( # lst permits later entries to refer to earlier entries
	...
	, zeroes_count = dat$zeroes_count

	# ones_count: count of ones in each cell
	, ones_count = dat$ones_count

	# yXc: which row in Xc is associated with each observation cell
	, yXc = yXc
