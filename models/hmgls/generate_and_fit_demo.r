...
nI_per_group = 1e1 # number of individuals, must be an integer >1
nY_per_ic = 1e1 # number of observations per individual/condition combo, must be an integer >1
...
nXc2 = nXc*2
nXg = 2^(nG_vars)
Z = matrix(rnorm(nXg*nXc2),nrow=nXg,ncol=nXc2)
iZc_sd = rweibull(nXc2,2,1)
iZc_r_mat = rethinking::rlkjcorr(1,nXc2,eta=1)
...
# compute gZc
gZc = matrix(NA,nrow=nXg,ncol=nXc2)
for(this_nXc2 in 1:nXc2){
	for(this_nXg in 1:nXg){
		gZc[this_nXg,this_nXc2] = Xg[this_nXg,] %*% Z[,this_nXc2]
	}
}
...
(
	...
	%>% summarise(
		{function(x){
			...
			this_dot = matrix(NA,nXc,2)
			for(this_nXc in 1:nXc){
				this_dot[this_nXc,1] = uXc[this_nXc,] %*% x[1:nXc,]
				this_dot[this_nXc,2] = uXc[this_nXc,] %*% x[(nXc+1):nXc2,]
			}
			tibble(
				c=1:nXc
				,m=this_dot[,1]
				,s=sqrt(exp(this_dot[,2]))
			)
		}}(cur_data())
		, .groups = 'keep'
	)
