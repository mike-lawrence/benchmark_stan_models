...
sim_pars = lst(
	...
	, num_vars = 2 #number of 2-level variables manipulated as crossed and within each subject, must be an integer >0
	, num_trials = 10 #number of trials per subject/condition combo, must be an integer >1

	#the rest of these you shouldn't touch
	, num_coef = 2*(2^(num_vars))
	, mu = rnorm(num_coef)
	...
	(
		...
		%>% summarise(
			(function(x){
				...
				out$loc = as.vector(contrast_matrix %*% t(x[,1:k]))
				out$scale = sqrt(exp(as.vector(contrast_matrix %*% t(x[,(k+1):k2]))))
				return(out)
			})(cur_data())
			, .groups = 'drop'
		)
